...
async function fetchEmail(firstName, lastName) {
  await new Promise((r) => setTimeout(r, 500));
  return `${firstName}.${lastName}@xxx.com`;
}
...
const EmailField = (props) => {
  ...
  React.useEffect(() => {
    ...
    if (firstName.trim() !== '' && lastName.trim() !== '') {
      fetchEmail(firstName, lastName).then((email) => {
        if (!!isCurrent) {
...
export const DependentFieldsWithAPI = () => {
  return (
    <>
